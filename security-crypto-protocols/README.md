...
Using this is **optional**. If you prefer starting from scratch, that’s completely fine.

Find the skeleton here:

https://github.com/celabshq/assignments/tree/main/security-crypto-protocols
...
- Create a repository on Github, Gitlab or something similar.
- When you have ideas or thoughts that would take too long to implement or you are uncertain about, add a comment in the code explaining it.
- Please submit your solution **at least a day before the scheduled interview** so that we have time to look at it.

We will evaluate your submission based on:

- **Correct Usage of Cryptography:** Correct utilization of cryptographic primitives to build the protocol.
- **Idiomatic Rust & Async Safety:** Clean utilization of Tokio tasks or similar, appropriate usage of `Arc`/`Mutex` or channels.
...
You can use AI to help understand the specification, the task, or any other part. But the code must not be generated by AI.

By submitting the assignment you attest to the fact that all code in the submission is written by you personally.

## 🚀 Getting Started - Skeleton

The skeleton can be used as a starting point. If you prefer to start from scratch, that’s fine too.
